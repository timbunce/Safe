#!./perl
$|=1;
BEGIN {
#    chdir 't' if -d 't';
#    @INC = '../lib';
    require Config; import Config;
    if ($Config{'extensions'} !~ /\bSafe\b/ && $Config{'osname'} ne 'VMS') {
        print "1..0\n";
        exit 0;
    }
}

use Safe qw(opdesc ops_to_opset opset_to_ops
		full_opset empty_opset MAXO define_optag);

print "1..$last_test\n";

# Set up a package namespace of things to be visible to the unsafe code
$Root::foo = "visible";
$bar = "invisible";

# Stop perl from moaning about identifies which are apparently only used once
$Root::foo .= "";
$bar .= "";

$cpt = new Safe "Root";
$cpt->reval(q{
    system("echo not ok 1");
});
if ($@ =~ /^system trapped by operation mask/) {
    print "ok 1\n";
} else {
    print "#$@" if $@;
    print "not ok 1\n";
}

$cpt->reval(q{
    print $foo eq 'visible' ? "ok 2\n" : "not ok 2\n";
    print $main::foo  eq 'visible' ? "ok 3\n" : "not ok 3\n";
    print defined($bar) ? "not ok 4\n" : "ok 4\n";
    print defined($::bar) ? "not ok 5\n" : "ok 5\n";
    print defined($main::bar) ? "not ok 6\n" : "ok 6\n";
});
print $@ ? "not ok 7\n#$@" : "ok 7\n";

$foo = "ok 8\n";
%bar = (key => "ok 9\n");
@baz = "o";
push(@baz, "10"); # Two steps to prevent "Identifier used only once..."
$glob = "ok 11\n";
@glob = qw(not ok 16);

$" = 'k ';

sub sayok12 { print "ok 12\n" }

$cpt->share(qw($foo %bar @baz *glob sayok12 $"));

$cpt->reval(q{
    print $foo ? $foo : "not ok 8\n";
    print $bar{key} ? $bar{key} : "not ok 9\n";
    if (@baz) {
	print "@baz\n";
    } else {
	print "not ok 10\n";
    }
    print $glob;
    sayok12();
    $foo =~ s/8/14/;
    $bar{new} = "ok 15\n";
    @glob = qw(ok 16);
});
print $@ ? "not ok 13\n#$@" : "ok 13\n";
$" = ' ';
print $foo, $bar{new}, "@glob\n";

$Root::foo = "not ok 17";
@{$cpt->varglob('bar')} = qw(not ok 18);
${$cpt->varglob('foo')} = "ok 17";
@Root::bar = "ok";
push(@Root::bar, "18"); # Two steps to prevent "Identifier used only once..."

print "$Root::foo\n";
print "@{$cpt->varglob('bar')}\n";

print MAXO() ? "ok 19\n" : "not ok 19\n";
print opdesc("gv") eq "glob value" ? "ok 20\n" : "not ok 20\n";

$m1 = $cpt->mask;
$cpt->trap("negate");
$m2 = $cpt->mask;
@masked = opset_to_ops($m1);
print $m2 eq ops_to_opset("negate", @masked) ? "ok 21\n" : "not ok 21\n";

print eval { $cpt->mask("a bad mask") } ? "not ok 22\n" : "ok 22\n";

print $cpt->reval("2 + 2") == 4 ? "ok 23\n" : "not ok 23\n";

$cpt->mask(empty_opset);
my $t_scalar = $cpt->reval('print wantarray ? "not ok 24\n" : "ok 24\n"');
print $cpt->reval('@ary=(6,7,8);@ary') == 3 ? "ok 25\n" : "not ok 25\n";
my @t_array  = $cpt->reval('print wantarray ? "ok 26\n" : "not ok 26\n"; (2,3,4)');
print $t_array[2] == 4 ? "ok 27\n" : "not ok 27\n";

my $t_scalar = $cpt->reval('die "foo bar"; 1');
print defined $t_scalar ? "not ok 28\n" : "ok 28\n";
print $@ =~ /foo bar/ ? "ok 29\n" : "not ok 29\n";

@empty_l = opset_to_ops(empty_opset);
print @empty_l == 0 ?   "ok 30\n" : "not ok 30\n";

@empty_l = opset_to_ops(ops_to_opset(':none'));
print @empty_l == 0 ?   "ok 31\n" : "not ok 31\n";

@full_l1  = opset_to_ops(full_opset);
print @full_l1 == MAXO() ? "ok 32\n" : "not ok 32\n";

@full_l2 = opset_to_ops(ops_to_opset(':all'));
print @full_l1 == @full_l2 ? "ok 33\n" : "not ok 33\n";
#print "@full_l1 == @full_l2\n";

# Check opname assertions
foreach(@full_l1) {
    die "bad opname: $_" if /\W/ or /^\d/;
}

$m1 = ops_to_opset(qw(padsv));
$m2 = ops_to_opset(qw(padsv padav !padav));
print $m1 eq $m2 ? "ok 34\n" : "not ok 34\n";

print eval { ops_to_opset(':newtag') } ? "not ok 35\n" : "ok 35\n";
define_optag(":newtag", ops_to_opset(qw(padsv padav padhv)));
print "ok 36\n";
print eval { ops_to_opset(':newtag') } ? "ok 37\n" : "not ok 37\n";

# --- opdesc

@desc = opdesc(':newtag','stub');
print "@desc" eq "private variable private array private hash stub"
				    ? "ok 38\n" : "not ok 38\n#@desc\n";
# --- rdo
  
$cpt->rdo('/non/existant/file.name');
print $! =~ /No such file/ ? "ok 39\n" : "not ok 39\n";
print 1 ? "ok 40\n" : "not ok 40\n#$@/$!\n";
  
$rdo_file = "tmp_rdo.tpl";
open X,">$rdo_file" or die "$rdo_file: $!";
print X "999\n";
close X;
$cpt->permit_only('const', 'leaveeval');
print $cpt->rdo($rdo_file) == 999 ? "ok 41\n" : "not ok 41\n#$@\n#$!\n";
unlink $rdo_file;

print "ok $last_test\n";
BEGIN { $last_test = 42 }
